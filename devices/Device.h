...
struct DevBasicInfo
{
	...
	DevBasicInfo()
	{
		memset(this,0,sizeof(DevBasicInfo));
	}
};
#pragma pack()
...
public:
	int  InitData(char* ptr = nullptr);				//初始化设备数据内存
	...
	void SetCriticalInterval(UINT32 msec);
	void SetImportantInterval(UINT32 msec);
	void SetUnimportantInterval(UINT32 msec);
...
protected:
	...
	UINT32 		  m_critical_interval;			//紧急数据查询间隔毫秒
	UINT32		  m_important_interval;			//正常数据查询间隔毫秒
	UINT32 		  m_unimportant_interval;		//不紧急数据查询间隔秒
